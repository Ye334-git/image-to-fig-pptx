      ...
      editablePreviewCancel.addEventListener("click", () => {
        ...
        if (editablePreviewProgressId) {
          fetchBackend(`/api/v1/progress/${encodeURIComponent(editablePreviewProgressId)}/cancel`, { method: "POST" }).catch(() => {});
        }
      ...
      async function createBackendManifest(options) {
        ...
        const response = await fetchBackend("/api/v1/images/generate", {
          method: "POST",
          headers: {
      ...
      async function createBackendImageEditManifest(options) {
        ...
        const response = await fetchBackend("/api/v1/images/edit", {
          method: "POST",
          headers: {
            "content-type": "application/json"
          },
        ...
        cutGrid.innerHTML = displayAssets.map((asset, index) => `
          <div class="cut-item${isSliceSelected(asset.id) ? " active" : ""}${asset.aiProcessing ? " ai-processing" : ""}${asset.auditFailed ? " audit-failed" : ""}" data-slice-id="${asset.id}" draggable="${asset.aiProcessing ? "false" : "true"}">
            ...
            <div class="cut-meta">
              ...
              <span class="cut-size">${asset.placement.width}px × ${asset.placement.height}px${formatSliceRadiiLabel(asset)}${asset.transparent ? " · 透明 PNG" : ""}${asset.auditFailed ? " · 审计失败 · 需手动调整" : ""}${asset.aiTransparent ? " · AI 透明" : ""}${getAiInpaintTypeLabel(asset) ? ` · ${getAiInpaintTypeLabel(asset)}` : ""}${asset.svgData ? " · SVG" : ""}${asset.aiRedrawn ? " · AI 重绘" : ""} · <span class="svg-recommendation ${getSvgRecommendation(asset).tone}" title="${escapeHtml(getSvgRecommendation(asset).reason)}">${escapeHtml(getSvgRecommendation(asset).label)}</span></span>
            </div>
        ...
        cutGrid.querySelectorAll("[data-svg-slice]").forEach((button) => {
          button.addEventListener("click", async (event) => {
            ...
            if (asset?.svgData && !asset.aiRedrawn) {
              ...
              const warning = shouldWarnBeforeSvgAction(asset, "local");
              if (warning && !window.confirm(`${warning}\n\n仍要执行本地 SVG 描摹吗？`)) return;
        ...
        cutGrid.querySelectorAll("[data-redraw-slice]").forEach((button) => {
          button.addEventListener("click", async (event) => {
            ...
            if (asset?.aiRedrawn && asset.svgData) {
              ...
              const warning = shouldWarnBeforeSvgAction(asset, "ai");
              if (warning && !window.confirm(`${warning}\n\n仍要执行 AI SVG 重绘吗？`)) return;
      ...
      function cancelSliceImageEditorAiComplete() {
        ...
        if (sliceImageEditorState.progressId) {
          fetchBackend(`/api/v1/progress/${encodeURIComponent(sliceImageEditorState.progressId)}/cancel`, { method: "POST" }).catch(() => {});
        }
      ...
      function cancelSliceAiRequest(id) {
        ...
        fetchBackend(`/api/v1/progress/${encodeURIComponent(request.progressId)}/cancel`, { method: "POST" }).catch(() => {});
      ...
      function startAiProgressPolling(progressId, onProgress) {
        ...
        const poll = async () => {
          if (stopped) return;
          try {
            const response = await fetch(`${PROXY_BASE_URL}/api/v1/progress/${encodeURIComponent(progressId)}`);
            if (response.ok) {
      ...
      function cancelBackgroundDecompositionPlanning() {
        ...
        fetchBackend(
          `/api/v1/progress/${encodeURIComponent(backgroundDecompositionRequest.progressId)}/cancel`,
          { method: "POST" }
        ).catch(() => {});
      ...
      async function runBackgroundDecompositionPlanning(forceRecognition = false) {
        ...
        try {
          const response = await fetchBackend("/api/v1/design/plan-background-decomposition", {
            method: "POST",
      ...
      async function makeSliceAiTransparent(id) {
        ...
        try {
          const restoreState = createSliceTransparencyRestoreState(asset);
          const sourceDataUrl = restoreState.dataUrl;
          const response = await fetchBackend("/api/v1/assets/ai-redraw", {
            method: "POST",
            signal: controller.signal,
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
      ...
      async function redrawSliceToSvg(id) {
        ...
        try {
          const response = await fetchBackend("/api/v1/assets/redraw-svg", {
            method: "POST",
            signal: controller.signal,
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
      ...
      async function vectorizeAssetWithBackend(dataUrl, width, height) {
        try {
          const response = await fetchBackend("/api/v1/assets/vectorize", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
      ...
      async function captureHtmlPreviewWithPlaywright(doc, screen) {
        ...
        const response = await fetchBackend("/api/v1/design/capture-figma", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      async function requestEditableDesignHtmlPreview(signal, progressId = "", localAssets = null) {
        ...
        const response = await fetchBackend("/api/v1/design/reconstruct-h5", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          signal,
          body: JSON.stringify({
      ...
      async function initializeLocalService() {
        ...
        startupMessage.textContent = "正在连接同源服务，并加载模型配置…";
        try {
          const healthResponse = await fetchAppWithTimeout(`${PROXY_BASE_URL}/api/v1/health`, {}, 5000);
          if (!healthResponse.ok) throw new Error(`本地服务异常：${healthResponse.status}`);
          const health = await healthResponse.json().catch(() => ({}));
          const highFidelityAvailable = health?.capabilities?.highFidelityCapture !== false;
          htmlPreviewHighFidelityCapture.disabled = !highFidelityAvailable;
          htmlPreviewHighFidelityCapture.title = highFidelityAvailable
            ? "使用系统 Chrome/Edge 进行高保真 DOM 捕获"
            : "未找到系统 Chrome/Edge，仅普通 DOM 捕获可用";
      ...
      async function renderWorkspaceDraftList() {
        ...
        try {
          const response = await fetchBackend("/api/v1/workspace-drafts");
      ...
      async function loadApiConfigFromBackend(throwOnError = false) {
        try {
          const response = await fetchAppWithTimeout(`${PROXY_BASE_URL}/api/v1/model-configs`, {}, 5000);
      ...
      async function saveTaskRouteSelection({ task, configId, restore = true }) {
        ...
        try {
          const response = await fetchBackend(`/api/v1/task-routing/${encodeURIComponent(task)}`, {
            method: "PUT",
            headers: { "content-type": "application/json" },
      ...
      async function saveModelConfigEditor(event) {
        ...
        try {
          const response = await fetchBackend(
            configId ? `/api/v1/model-configs/${encodeURIComponent(configId)}` : "/api/v1/model-configs",
            {
      ...
      async function testModelConfigCard(configId, button) {
        ...
        try {
          const response = await fetchBackend(`/api/v1/model-configs/${encodeURIComponent(configId)}/test`, {
            method: "POST"
          });
          const data = await response.json().catch(() => ({}));
      ...
      async function testEditedModelConfig() {
        ...
        apiConfigStatus.textContent = "正在测试配置…";
        try {
          const response = await fetchBackend("/api/v1/model-configs/preview/test", {
            method: "POST",
            headers: { "content-type": "application/json" },
      ...
      async function deleteEditedModelConfig() {
        ...
        try {
          const response = await fetchBackend(`/api/v1/model-configs/${encodeURIComponent(configId)}`, {
            method: "DELETE"
          });
          const data = await response.json().catch(() => ({}));
      ...
      async function downloadEditedModelConfigModels() {
        ...
        try {
          const response = await fetchBackend("/api/v1/model-configs/preview/models", {
            method: "POST",
            headers: { "content-type": "application/json" },
            body: JSON.stringify(buildEditedModelConfigPreviewRequest())
          });
