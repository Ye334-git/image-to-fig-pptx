...
function buildWorkspaceDraftSnapshot({
  ...
  return {
    schema: "image-to-html.workspace.v1",
    version: 1,
    savedAt: now(),
...
async function readWorkspaceDraft({ fetchWithTimeout }) {
  const response = await fetchWithTimeout(`${workspaceProxyBaseUrl}/api/v1/workspace-draft`, {}, 30000);
...
async function writeWorkspaceDraft({ draft, draftId }, { fetchBackend }) {
  const response = await fetchBackend("/api/v1/workspace-draft", {
    method: "POST",
...
async function deleteWorkspaceDraft({ fetchBackend }) {
  const response = await fetchBackend("/api/v1/workspace-draft", { method: "DELETE" });
  if (!response.ok) throw new Error(`HTTP ${response.status}`);
}

async function saveWorkspaceRestorePreference(choice, { fetchBackend }) {
  const response = await fetchBackend("/api/v1/workspace-restore-preference", {
    method: "POST",
    headers: { "content-type": "application/json" },
...
async function deleteWorkspaceDraftById(id, { fetchBackend }) {
  const response = await fetchBackend(`/api/v1/workspace-drafts/${encodeURIComponent(id)}`, { method: "DELETE" });
  return response.ok;
}

async function updateWorkspaceDraftNote(id, note, { fetchBackend }) {
  const response = await fetchBackend(`/api/v1/workspace-drafts/${encodeURIComponent(id)}/note`, {
    method: "POST",
    headers: { "content-type": "application/json" },
...
async function loadWorkspaceDraftItem(id, { fetchBackend }) {
  const response = await fetchBackend(`/api/v1/workspace-drafts/${encodeURIComponent(id)}`);
  const payload = await response.json().catch(() => ({}));
...
async function duplicateWorkspaceDraft(id, { fetchBackend }) {
  const response = await fetchBackend(`/api/v1/workspace-drafts/${encodeURIComponent(id)}/duplicate`, { method: "POST" });
  const payload = await response.json().catch(() => ({}));
