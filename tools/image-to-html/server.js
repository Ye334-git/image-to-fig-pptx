...
const DATA_DIR = path.resolve(
  process.env.IMAGE_TO_HTML_DATA_DIR || path.join(__dirname, ".image-to-html-data")
);
const CONFIG_FILE = path.join(DATA_DIR, "model-config.json");
const WORKSPACE_HISTORY_DIR = path.join(DATA_DIR, "workspaces");
const UI_HTML_FILE = path.join(__dirname, "dist", "ui.html");
fs.mkdirSync(DATA_DIR, { recursive: true });
const FIGMA_CAPTURE_RUNTIME = fs.readFileSync(
  path.join(__dirname, "src/vendor/figma-capture.js"),
  "utf8"
);
const SYSTEM_BROWSER_LAUNCH_OPTIONS = resolveSystemBrowserLaunchOptions();
const playwrightFigmaCaptureService = createPlaywrightFigmaCaptureService({
  chromium,
  captureRuntime: FIGMA_CAPTURE_RUNTIME,
  launchOptions: SYSTEM_BROWSER_LAUNCH_OPTIONS || {}
});
...
const handleDesignRoutes = createDesignRoutes({
  ...
  captureHighFidelityFigma: (payload) => {
    if (!SYSTEM_BROWSER_LAUNCH_OPTIONS) {
      throw Object.assign(new Error("未找到系统 Chrome/Edge，高保真捕获不可用"), { statusCode: 503 });
    }
    return playwrightFigmaCaptureService.capture(payload);
  },
  exportFigManifest,
  sendBinary,
  sendJson
});
const handleExportRoutes = createExportRoutes({
  readJson,
  exportFigManifest,
  sendBinary
});
...
function startServer() {
  if (server.listening) return ready;
  server.once("error", rejectReady);
  server.listen(PORT, HOST, () => {
    console.log(`image-to-html listening on http://${HOST}:${PORT}`);
...
function sendUiHtml(response) {
  if (!fs.existsSync(UI_HTML_FILE)) {
    sendJson(response, 503, { error: "Web UI 尚未构建，请先运行 npm run build" });
    return;
  }
  ...
  fs.createReadStream(UI_HTML_FILE).pipe(response);
}

function resolveSystemBrowserLaunchOptions() {
  const explicit = process.env.IMAGE_TO_HTML_BROWSER_PATH
  ...
  return executablePath ? { executablePath, headless: true } : null;
}
